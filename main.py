    ...
    try:
        ...
        if file_foto:
            ...
            if res.status_code == 200:
                ...
                # Mengambil URL Utuh dari Telegram API
...
def simpan_ke_gsheets(data_baru, foto_url=None):
    ...
    try:
        ...
        # PERBAIKAN CARA 1:
        # Menambahkan tanda petik tunggal (') di awal URL
        # agar Google Sheets menyimpannya sebagai Teks Murni (Raw String Utuh)
        ...
        row = [
            ...
            data_baru["Koordinat"],
            data_baru["Link_Maps"],
            url_simpan  # Disimpan sebagai Teks Murni
        ]
        
        # Menggunakan value_input_option="USER_ENTERED"
...
# --------------------------
    ...
    try:
        ...
        if not df_laporan.empty:
            ...
            if not df_laporan.empty:
                # Memastikan URL foto dibaca secara utuh
                ...
                # Menggunakan ImageColumn untuk langsung memuat URL Teks Murni
                st.dataframe(
